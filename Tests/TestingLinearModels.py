...
plot1d(X, 2 * X.squeeze() + torch.normal(0, 0.1, size=(100,)), model2.predict(X), "Ridge regression with all weight on first 2 samples")
...
weight[:50] = 1
temp = 2 * X.squeeze() + torch.normal(0, 0.1, size=(100,))
temp[:50] = 5
history = model4.fit(X, temp, sample_weight=weight, epochs=10000, metrics=["rmse"])
summary(model4.predict(X), 2 * X.squeeze(), 1)
plot1d(X, temp, model4.predict(X), "Elasticnet regression")
