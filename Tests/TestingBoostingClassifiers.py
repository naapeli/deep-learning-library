...
from src.DLL.MachineLearning.SupervisedLearning.Trees import GradientBoostingClassifier, AdaBoostClassifier, DecisionTree
...
if n_classes == 2:
    ...
    ax[0].set_title(f"gradient boosting ROC, auc = {auc(fpr, tpr)}, sklearn = {sk_auc(fpr, tpr)}")
    ax[0].plot([0, 1], [0, 1])
    ax[0].step(fpr, tpr)
    ax[0].set_xlabel("False positive rate")
    ax[0].set_ylabel("True positive rate")

    ax[1].set_title(f"ada boost ROC, auc = {auc(fpr2, tpr2)}, sklearn = {sk_auc(fpr2, tpr2)}")
    ax[1].plot([0, 1], [0, 1])
    ax[1].step(fpr2, tpr2)
