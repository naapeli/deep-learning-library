...
class _Compound(_Base):
    ...
    def update(self, derivative_function, X, noise=0, epsilon=1e-5):
        if self.add:
            ...
            kernel_1_covariance = torch.tensor([[self.kernel_1(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
            kernel_2_covariance = torch.tensor([[self.kernel_2(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
...
class SquaredExponentialCovariance(_Base):
    def __init__(self, sigma=1.0, correlation_length=1.0):
    ...
    def update(self, derivative_function, X, noise=0, epsilon=1e-5):
        derivative_covariance_sigma = torch.tensor([[self.derivative_sigma(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        derivative_covariance_corr_len = torch.tensor([[self.derivative_corr_len(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
...
class LinearCovariance(_Base):
    ...
    def update(self, derivative_function, X, noise=0, epsilon=1e-5):
        derivative_covariance_sigma = torch.tensor([[self.derivative_sigma(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        derivative_covariance_sigma_bias = torch.tensor([[self.derivative_sigma_bias(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        self.sigma.grad = derivative_function(derivative_covariance_sigma).to(dtype=self.sigma.dtype).squeeze(0)
...
class PeriodicCovariance(_Base):
    ...
    def update(self, derivative_function, X, noise=0, epsilon=1e-5):
        derivative_covariance_sigma = torch.tensor([[self.derivative_sigma(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        derivative_covariance_corr_len = torch.tensor([[self.derivative_corr_len(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        derivative_covariance_period = torch.tensor([[self.derivative_period(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        self.sigma.grad = derivative_function(derivative_covariance_sigma).to(dtype=self.sigma.dtype).squeeze(0)
        self.correlation_length.grad = derivative_function(derivative_covariance_corr_len).to(dtype=self.correlation_length.dtype).squeeze(0)
...
class RationalQuadraticCovariance(_Base):
    ...
    def update(self, derivative_function, X, noise=0, epsilon=1e-5):
        derivative_covariance_sigma = torch.tensor([[self.derivative_sigma(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        derivative_covariance_corr_len = torch.tensor([[self.derivative_corr_len(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        derivative_covariance_alpha = torch.tensor([[self.derivative_alpha(x1, x2) for x1 in X] for x2 in X], dtype=X.dtype).T + (noise + epsilon) * torch.eye(len(X))
        self.sigma.grad = derivative_function(derivative_covariance_sigma).to(dtype=self.sigma.dtype).squeeze(0)
        self.correlation_length.grad = derivative_function(derivative_covariance_corr_len).to(dtype=self.correlation_length.dtype).squeeze(0)
